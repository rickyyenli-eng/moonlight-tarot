...
// ===== 問題模板 =====
// 每個模板都有 make(vals)：填空全部選填，沒填就用通用問法，絕不會空白
const TAROT_TEMPLATES = [
  { label: '💼 工作狀態', blanks: ['公司／單位名稱', '職位'],
    make: v => `我想要知道未來一個月${v[0] ? `在「${v[0]}」` : ''}${v[1] ? `擔任「${v[1]}」` : ''}的工作狀態如何？` },
  { label: '🔍 找工作', blanks: [],
    make: () => '我想要知道未來一個月找工作的狀況？' },
  { label: '🌸 桃花', blanks: [],
    make: () => '我想要知道未來一個月的桃花狀況？' },
  { label: '💑 感情相處', blanks: ['對方的名字或稱呼'],
    make: v => `我想要知道未來一個月${v[0] ? `與「${v[0]}」` : '與對方'}的相處狀況？` },
  { label: '🌈 整體運勢', blanks: [],
    make: () => '我想要知道未來一個月的整體運勢？' },
  { label: '💰 財運', blanks: [],
    make: () => '我想要知道未來一個月的財運？' },
  { label: '⚖️ 二擇一', blanks: [], spread: 'choice',
    make: () => '我正在兩個選擇之間難以抉擇，想知道兩條路各自的發展？' },
];
