...
const TAROT_TEMPLATES = [
  {
    label: '💼 工作狀態',
    blanks: ['公司／單位名稱', '你的職位'],
    make: v => `我想要知道未來一個月${v[0] ? `在「${v[0]}」` : ''}${v[1] ? `擔任「${v[1]}」` : ''}的工作狀態如何？`,
  },
  {
    label: '🔍 找工作',
    blanks: ['想找的產業或職務，例：行銷企劃'],
    make: v => `我想要知道未來一個月找${v[0] ? `「${v[0]}」相關` : ''}工作的狀況？`,
  },
  {
    label: '🌸 桃花',
    blanks: ['目前的狀態，例：單身、曖昧中'],
    make: v => `我想要知道未來一個月的桃花狀況${v[0] ? `（目前${v[0]}）` : ''}？`,
  },
  {
    label: '💑 感情相處',
    blanks: ['對方的名字或稱呼'],
    make: v => `我想要知道未來一個月${v[0] ? `與「${v[0]}」` : '與對方'}的相處狀況？`,
  },
  {
    label: '🌈 整體運勢',
    blanks: ['最近最在意的事，例：搬家'],
    make: v => `我想要知道未來一個月的整體運勢${v[0] ? `，特別是「${v[0]}」這方面` : ''}？`,
  },
  {
    label: '💰 財運',
    blanks: ['想問的方向，例：投資、副業'],
    make: v => `我想要知道未來一個月的財運${v[0] ? `，特別是「${v[0]}」方面` : ''}？`,
  },
  {
    label: '⚖️ 二擇一',
    blanks: ['選擇 A，例：留在現在的公司', '選擇 B，例：接受新工作'],
    spread: 'choice',
    make: v => (v[0] || v[1])
      ? `我在「${v[0] || '選擇 A'}」與「${v[1] || '選擇 B'}」之間難以抉擇，想知道兩條路各自的發展？`
...
// 牌義以傳統含意為基礎，改寫成「現在可以做什麼」的行動指引。
function oracleImg(n) { return 'oracle/o' + String(n).padStart(2, '0') + '.jpg'; }

const ORACLE_CARDS = [
  { n: 1, title: '騎士', de: 'Der Reiter', msg: '有消息正在路上。與其乾等，不如主動去問那一句。' },
  { n: 2, title: '三葉草', de: 'Der Klee', msg: '小小的好運就在附近。別錯過那些看起來不起眼的機會。' },
  { n: 3, title: '船', de: 'Das Schiff', msg: '有些事得離開原地才看得見。跨出熟悉的範圍一點點。' },
  { n: 4, title: '房屋', de: 'Das Haus', msg: '先把自己的地基顧好。家裡穩了，外面才走得遠。' },
  { n: 5, title: '樹', de: 'Der Baum', msg: '這件事需要時間長。照顧好身體，別跟它比快。' },
  { n: 6, title: '雲', de: 'Die Wolken', msg: '現在看不清楚是正常的。等霧散一點再決定，別急著下判斷。' },
  { n: 7, title: '蛇', de: 'Die Schlange', msg: '路沒有你想的那麼直。留意那個繞著你打轉的人或事。' },
  { n: 8, title: '棺材', de: 'Der Sarg', msg: '有些東西該讓它結束了。放手不是失去，是騰出空間。' },
  { n: 9, title: '花束', de: 'Die Blumen', msg: '有份好意正朝你來。記得說謝謝，也記得欣賞自己。' },
  { n: 10, title: '鐮刀', de: 'Die Sense', msg: '拖著只會更痛。該切的乾脆切，一次處理完。' },
  { n: 11, title: '鞭子', de: 'Die Rute', msg: '同樣的爭執重複幾次了？換個講法，或乾脆先不講。' },
  { n: 12, title: '鳥', de: 'Die Vögel', msg: '話太多了，包括你腦子裡那些。挑重要的講，其他放掉。' },
  { n: 13, title: '孩子', de: 'Das Kind', msg: '用初學者的心態重來一次。不會沒關係，開始就好。' },
  { n: 14, title: '狐狸', de: 'Der Fuchs', msg: '有件事不像表面那樣。多問一句，多查一下再說。' },
  { n: 15, title: '熊', de: 'Der Bär', msg: '你比想像中有本錢。也別忘了身邊那個罩得住你的人。' },
  { n: 16, title: '星星', de: 'Die Sterne', msg: '抬頭看一下你原本要去的方向。它還在那裡。' },
  { n: 17, title: '鸛鳥', de: 'Der Storch', msg: '該換位置了。不一定是搬家，換個作法也算。' },
  { n: 18, title: '狗', de: 'Der Hund', msg: '去找那個一直都在的人。你不用什麼都自己扛。' },
  { n: 19, title: '高塔', de: 'Der Turm', msg: '給自己一段獨處的時間，也把該立的界線立起來。' },
  { n: 20, title: '花園', de: 'Der Garten', msg: '走出去見人。有些機會只在人群裡出現。' },
  { n: 21, title: '山', de: 'Der Berg', msg: '前面確實有座山。繞路不丟臉，硬爬才傷身。' },
  { n: 22, title: '岔路', de: 'Der Weg', msg: '路口到了。與其求最好，不如選你願意負責的那條。' },
  { n: 23, title: '老鼠', de: 'Die Mäuse', msg: '有東西在慢慢消耗你。找出那個破口，補起來。' },
  { n: 24, title: '心', de: 'Das Herz', msg: '跟著心走一次。你其實知道自己在乎什麼。' },
  { n: 25, title: '戒指', de: 'Der Ring', msg: '先確認這個承諾是不是你真心要的，再點頭。' },
  { n: 26, title: '書', de: 'Das Buch', msg: '有些事還沒攤開。該學的去學，該問的去問。' },
  { n: 27, title: '信', de: 'Der Brief', msg: '把話寫下來會比用講的清楚。今天就傳出去。' },
  { n: 28, title: '男人', de: 'Der Herr', msg: '有個人在這件事裡份量很重。想清楚你要不要靠近。' },
  { n: 29, title: '女人', de: 'Die Dame', msg: '有個人在這件事裡份量很重。想清楚你要不要靠近。' },
  { n: 30, title: '百合', de: 'Die Lilien', msg: '慢下來。這件事需要的是沉穩，不是熱度。' },
  { n: 31, title: '太陽', de: 'Die Sonne', msg: '你的狀態正好。想做的事趁現在，別拖到天黑。' },
  { n: 32, title: '月亮', de: 'Der Mond', msg: '你的感受是真的，值得被說出來。別再壓著。' },
  { n: 33, title: '鑰匙', de: 'Der Schlüssel', msg: '答案就在你手上，只是還沒去轉那一下。' },
  { n: 34, title: '魚', de: 'Die Fische', msg: '把金錢和時間的流向理一遍。省下的比賺到的實在。' },
  { n: 35, title: '錨', de: 'Der Anker', msg: '撐住。你已經比想像中更接近終點了。' },
...
// ===== 綜合解答（第二段用）=====
function synthesize(spreadId, draws, question, v) {
  v = v || 0;
  ...
  if (question) paras.push(V([
    `針對你問的「${question}」，把${draws.length}張牌湊起來看，是這樣回答的：`,
    `你問的是「${question}」。這${draws.length}張牌合起來，給的答案是這樣：`,
    `就「${question}」這件事，牌面是這樣說的：`,
  ], v));
  ...
  paras.push(V((TOPIC_CLOSE[topic.id] || TOPIC_CLOSE.general)[lbl], v));
  return paras;
}
...
// ===== 完整報告 =====
function buildReport({ who, question, spreadId, draws, date, variant }) {
  const spread = SPREADS[spreadId];
  const topic = detectTopic(question);
  const L = [];
  ...
  L.push('【二、綜合解答】');
  L.push('');
  synthesize(spreadId, draws, question, variant).forEach(p => { L.push(p); L.push(''); });
  L.push('━━━━━━━━━━━━━━━━');
